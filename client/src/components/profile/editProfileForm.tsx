...
const EditProfileForm: React.FC<Props> = () => {
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        // if (validateForm()) {
        try {
            const response = await axios.post(`${baseUrl}/company/${id}`, formData);
            ...
            <form onSubmit={handleSubmit} className="flex flex-wrap justify-between items-center gap-5">
                ...
                <div className="lg:w-[47%] w-full">
                    {formData.companyLogo && typeof formData.companyLogo === 'string' ?
                        <Image src={formData.companyLogo} alt='Profile Image' width={20} height={20} className='mx-auto w-40 h-40 rounded-full' /> :
